...
router.post("/signup", async (req, res) => {
  ...
  const { username, email, contactNumber, password }:SignupType = userInput.data;
  ...
  try{
    ...
    const token = jwt.sign({id: status._id}, USER_SECRET_KEY, {expiresIn: "1hr"});
...
router.post("/login", async (req, res) => {
  ...
  if(!isUser) {
    ...
    const token = jwt.sign({id: isUser._id}, USER_SECRET_KEY, { expiresIn: "1hr" });
...
router.get('/purchasedCourses', authenticateUser, async (req, res) => {
  const user = await User.findOne({_id: req.headers["userId"] }).populate('purchasedCourses');
  if (user) {
